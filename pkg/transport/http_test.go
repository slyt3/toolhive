...
func TestShouldEnableHealthCheck(t *testing.T) {
	...
	// Health checks are always enabled for both local and remote workloads.
	assert.True(t, shouldEnableHealthCheck(false), "should be enabled for local workloads")
	assert.True(t, shouldEnableHealthCheck(true), "should be enabled for remote workloads")
}
